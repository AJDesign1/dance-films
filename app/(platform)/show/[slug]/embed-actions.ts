...
export async function getEmbedUrl(kind: "full" | "perf", id: string): Promise<string | null> {
  ...
  if (kind === "perf") {
    const { data } = await supabase
      ...
      .eq("id", id)
      .maybeSingle();
    return data?.bunny_video_id ? bunnyEmbedUrl(data.bunny_video_id, { autoplay: true }) : null;
  }
  ...
  const { data } = await supabase
    ...
    .eq("show_id", id)
    .maybeSingle();
  return data?.full_show_bunny_video_id ? bunnyEmbedUrl(data.full_show_bunny_video_id, { autoplay: true }) : null;
}
