...
/** Grid poster card (3:4) with owned → Watch / not-owned → Buy states. */
export default function ShowCard({ show }: { show: ShopShow }) {
  return (
    <Link href={`/show/${show.slug}`} className={styles.cardLink}>
      <div
        style={{
          position: "relative",
          aspectRatio: "3 / 4",
      ...
      >
        ...
        {/* media badge (all V1 shows are video) */}
        ...
        <div style={{ position: "absolute", left: 13, right: 13, bottom: 13, pointerEvents: "none" }}>
          <div style={{ fontFamily: "var(--disp)", fontWeight: 800, fontSize: 38, letterSpacing: ".01em", textTransform: "uppercase", color: "#fff", lineHeight: ".88" }}>
            {show.title}
          </div>
